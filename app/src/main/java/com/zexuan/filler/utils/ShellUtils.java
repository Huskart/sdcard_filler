...
public class ShellUtils {
    ...
    public static void volumeReduce() {
        new Thread(new Runnable() {
            @Override
            public void run() {
                Instrumentation m_Instrumentation = new Instrumentation();
                m_Instrumentation.sendKeyDownUpSync(KeyEvent.KEYCODE_VOLUME_DOWN);
            }
        }).start();
    }
    ...
    public static void killProcess(String packageName) {
        if (ShellUtils.getPid(packageName) != -1) {
            int pid;
            pid = ShellUtils.getPid(packageName);
            if (!(pid == -1 || pid == 0)) {
